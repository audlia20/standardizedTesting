...
for(i in 2:length(df_list)) {
  all_data <- merge(all_data, df_list[[i]], by = key_columns, all = TRUE)
}
columns <- c(key_columns)
...
for(column in standardized) {
  complete <- complete & !is.na(all_data[[column]])
}


cleaned_data <- all_data[complete, ]
...
rows_with_NAs <- rep(FALSE, nrow(filtered_data))
